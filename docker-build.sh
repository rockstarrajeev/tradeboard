...
test_python_deps() {
    ...
    if docker-compose exec -T tradeboard python -c "import numba; import llvmlite; print('SUCCESS')" 2>/dev/null | grep -q "SUCCESS"; then
        print_success "numba, llvmlite imports successful"
    else
        print_error "Failed to import dependencies"
        print_info "Running detailed test..."
        docker-compose exec tradeboard python -c "import numba; import llvmlite; print('SUCCESS')"
        return 1
    fi
