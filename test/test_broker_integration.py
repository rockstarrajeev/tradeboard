...
a *running* Tradeboard instance, for **any** broker, via the public ``/api/v1/*``
...
Usage:
    # 1) Start Tradeboard and log into the broker first, then:
...
class Runner:
    ...
    def test_symbols(self):
        ...
        for sym, exch in checks:
            ...
            self.record("symbols", f"equity symbol '{sym}' in Tradeboard format", "PASS" if match else "FAIL",
...
class _WSClient:
    """Minimal Tradeboard WebSocket-proxy client for LTP/Quote/Depth subscription tests."""
...
def main():
    parser = argparse.ArgumentParser(description="Tradeboard broker-integration test runner")
    parser.add_argument("--apikey", default=None, help="Tradeboard API key (else API_KEY env/.env)")
    ...
    print(f"{B}Tradeboard Broker Integration Test Runner{RST}")
