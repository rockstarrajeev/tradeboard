...
Tradeboard combines application authentication, hashed API keys, request rate limits, IP-ban middleware, and optional TOTP. The Security dashboard manages abuse thresholds and IP bans; it is not a general firewall or CIDR whitelist.
...
By default, Tradeboard uses the immediate network peer as the client IP. Set `TRUST_PROXY_HEADERS=True` only when a controlled reverse proxy is the sole route to the application. In that mode Tradeboard can use headers such as `CF-Connecting-IP`, `True-Client-IP`, `X-Real-IP`, and `X-Forwarded-For`.
...
- Tradeboard API keys are stored as hashes with the installation's pepper.
...
- Regenerate an Tradeboard API key if it is exposed, then update every client that used it.
...
The current Tradeboard middleware has no CIDR allowlist feature. Use a host firewall, cloud security group, VPN, or reverse-proxy access policy when network allowlisting is required.

## Recommended Baseline

1. Use a unique application password and enable Tradeboard TOTP.
