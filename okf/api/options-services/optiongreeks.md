---
type: API Endpoint
title: OptionGreeks
description: Calculate option Greeks and implied volatility
resource: https://github.com/rockstarrajeev/tradeboard/blob/main/docs/api/options-services/optiongreeks.md
tags:
- options
- greeks
- implied-volatility
- black-76
timestamp: '2026-07-03T00:00:00+00:00'
---

> **Map entry — no duplicated content.** The authoritative text lives in the
> source doc linked below; this OKF concept exists only to index, tag, and
> cross-link it. Edit the source, never this file.

Calculate option Greeks and implied volatility.

**Source:** [docs/api/options-services/optiongreeks.md](../../../docs/api/options-services/optiongreeks.md)

**Related:** [overview](../../overview.md) · [order constants](../../sdk/order-constants.md) · [symbol format](../../sdk/symbol-format.md)
