---
type: API Endpoint
title: CancelOrder
description: Cancel a specific open order by order id
resource: https://github.com/rockstarrajeev/tradeboard/blob/main/docs/api/order-management/cancelorder.md
tags:
- order
- cancel
- trading
timestamp: '2026-07-03T00:00:00+00:00'
---

> **Map entry — no duplicated content.** The authoritative text lives in the
> source doc linked below; this OKF concept exists only to index, tag, and
> cross-link it. Edit the source, never this file.

Cancel a specific open order by order id.

**Source:** [docs/api/order-management/cancelorder.md](../../../docs/api/order-management/cancelorder.md)

**Related:** [overview](../../overview.md) · [order constants](../../sdk/order-constants.md) · [symbol format](../../sdk/symbol-format.md)
